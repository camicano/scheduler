...
function timeSlotTemplate(value, hour, client_class) {
  return  "<div class='time_slot " + client_class + "' value='" + value + "'>" +
    				hour +
    			"</div>";
}

function allTimeSlotsTemplate(title, time_slots) {
...
;
function addMonthSelectBinding() {
	var $monthSelect = $('#month_select');
	var $available_times = $('#available_times');
	var client_id = $('#client').val();
	var coach_id = $('#coach').val();

	$monthSelect.on('change', function() {
		$.get('/time_slots/' + $monthSelect.val() + '/coach/' + coach_id, function( data ) {
			...
			$.each(data, function(title, day) {
				...
				$.each(day, function(index, time_slot) {
					var formatted_hour = convertHourFormat(time_slot.hour);
					
					if( time_slot.client_id && time_slot.client_id.toString() === client_id ) {
						all_time_slot_templates += timeSlotTemplate(time_slot.id, formatted_hour, 'client_appt');
					} else {
						all_time_slot_templates += timeSlotTemplate(time_slot.id, formatted_hour, '');
					}
				});

				$available_times.append(allTimeSlotsTemplate(title, all_time_slot_templates));
	...
	$time_slot.on('click', function(e) { 
		...
		$.post( '/time_slots/' + id, function( data ) {
  		...
  		if ( data.client_id !== null ) {
  			$this.addClass('client_appt');
  		} else {
  			$this.removeClass('client_appt');
  		}
		});
	});
}
